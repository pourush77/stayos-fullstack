...
function holdDetailForRoomType(holds: GroupHoldDto[], roomTypeName?: string) {
  ...
  const matchingBlocks = matchingHold.roomBlocks.filter(
    (block) => block.roomTypeName === roomTypeName,
  );
...
export function AvailabilityCalendarPage() {
  ...
  const endDate = useMemo(
    () => new Date(startDate.getTime() + (rangeDays - 1) * DAY_MS),
    [startDate, rangeDays],
  );
  const days = useMemo(
    () => Array.from({ length: rangeDays }, (_, i) => new Date(startDate.getTime() + i * DAY_MS)),
    [startDate, rangeDays],
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const active =
          properties.find(
            (p) => (typeof p.status === 'string' ? p.status.toUpperCase() : 'ACTIVE') === 'ACTIVE',
          ) ?? properties[0];
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([roomsData, roomTypesData, resDataRaw, holdsData]) => {
            ...
            .map(
              (roomType) =>
                [
                  ...
                  getString(
                    roomType,
                    ['name', 'displayName', 'title', 'label', 'code'],
                    'Room type',
                  ),
                ] as const,
            )
        ...
        setGroupHolds(
          holdsData.filter((hold) => hold.status === 'ON_HOLD' || hold.status === 'CONFIRMED'),
        );
  ...
  const todayArrivals = reservations.filter(
    (r) => r.arrivalDate === todayIso && r.status !== 'CANCELLED',
  ).length;
  const todayDepartures = reservations.filter(
    (r) => r.departureDate === todayIso && r.status !== 'CANCELLED',
  ).length;
  const availableTonight = rooms.filter(
    (room) => isReadyRoom(room.operationalStatus) && !isCellOccupied(room.id, new Date()),
  ).length;
  const readyRooms = rooms.filter((room) => isReadyRoom(room.operationalStatus)).length;
  const roomTypeOptions = Array.from(
    new Set(rooms.map((room) => room.roomTypeName ?? 'Room type')),
  ).sort();
  const visibleRooms =
    roomTypeFilter === 'all'
      ? rooms
  ...
  const availableCountForDay = (day: Date) =>
    visibleRooms.filter(
      (room) => isReadyRoom(room.operationalStatus) && !isCellOccupied(room.id, day),
    ).length;
  ...
  const firstGroupHoldId = groupHolds
    .map((hold) =>
      getString(hold as unknown as Record<string, unknown>, [
        'id',
        'groupBookingId',
        '_id',
        'uuid',
      ...
      <Stack gap={spacing[3]}>
        <Group justify="space-between" align="flex-end">
          <Stack gap={4}>
            ...
            <Text c="#64748b" size="sm">
              {isoDay(startDate)} to {isoDay(endDate)} - {rooms.length} rooms
            </Text>
          </Stack>
          <Group gap={8}>
            <Button
              ...
              onClick={() => setOffsetDays((o) => o - rangeDays)}
            >
              Prev
            </Button>
            <Button
            ...
            <Button component={Link} href="/reservations/new" color="stayosBrand" size="sm">
              New Booking
            </Button>
          </Group>
        </Group>
        ...
        <SimpleGrid cols={{ base: 1, sm: 2, lg: 5 }} spacing={spacing[2]}>
          {[
            ...
            {
              label: 'Group holds',
              value: groupHolds.length,
              detail: groupHolds.length === 1 ? 'Open hold' : 'View holds',
              icon: <Users size={17} />,
              tone: '#dc2626',
              bg: '#fef2f2',
              href: groupHoldsHref,
            ...
            >
              <Group gap={10} wrap="nowrap">
                ...
                <Box style={{ minWidth: 0 }}>
                  <Text c="#111827" fw={850} style={{ fontSize: 18, lineHeight: '22px' }}>
                    {item.value}
                  </Text>
                  <Text c="#334155" fw={700} style={{ fontSize: 12, lineHeight: '16px' }}>
                    {item.label}
                  </Text>
                  <Text c="#64748b" style={{ fontSize: 11, lineHeight: '15px' }}>
                    {item.detail}
                  </Text>
                </Box>
              </Group>
          ...
          <Group justify="space-between" align="center" gap={spacing[3]}>
            <Box>
              <Text c="#101828" fw={800} style={{ fontSize: 15 }}>
                Room readiness by type
              </Text>
              <Text c="#64748b" size="xs">
                Physical-room view for assignment planning. Use Check Availability for sellable
                inventory.
              </Text>
            </Box>
            <Group gap={8}>
        ...
        <Group justify="space-between" align="center">
          <Text c="#334155" fw={700} size="sm">
            Click an empty cell to start a booking for that room type and date.
          </Text>
          <Text c="#64748b" size="xs">
            Empty cells show physical-room assignment space; final availability is checked in the
            booking flow.
          </Text>
        </Group>
        ...
        <Paper
          ...
          style={{
            background: '#ffffff',
            border: '1px solid #e2e8f0',
            maxHeight: 'clamp(520px, 62vh, 760px)',
            overflow: 'auto',
                  ...
                  return (
                    <Box
                      ...
                      style={{
                        ...
                        background: isToday ? '#f6f1ff' : undefined,
                        boxShadow: isToday ? 'inset 0 -3px 0 #7d4dd6' : undefined,
                      }}
                    >
                      <Text size="xs" c={isToday ? '#6536b5' : '#64748b'} fw={700}>
                        {lbl.top}
                      </Text>
                      <Text size="sm" c="#101828" fw={800}>
                        {lbl.bottom}
                      </Text>
                    </Box>
                  );
                  ...
                  return (
                    <Box
                      ...
                      style={{
                        ...
                        textAlign: 'center',
                        borderRight: '1px solid #eef2f7',
                        background: holds.length ? '#fff7ed' : '#ffffff',
                      }}
                    >
                      <Text
                        c={count > 0 ? '#166534' : '#64748b'}
                        fw={850}
                      ...
                      {holds.length ? (
                        <Text
                          c="#9a3412"
                          style={{ fontSize: 9, fontWeight: 800, lineHeight: '12px' }}
                        >
                          HOLD
                        </Text>
                      ) : null}
              ...
              {visibleRooms.map((room) => (
                ...
                >
                  ...
                  >
                    ...
                    {!isReadyRoom(room.operationalStatus) ? (
                      <Text c="#b45309" size="xs" fw={700}>
                        {statusKey(room.operationalStatus).replace(/_/g, ' ') || 'NOT READY'}
                      </Text>
                    ) : null}
                  </Box>
                  ...
                  <Box style={{ position: 'relative', display: 'flex', flex: 1 }}>
                    {days.map((d, i) => {
                      ...
                      const roomReady = isReadyRoom(room.operationalStatus);
                      ...
                      const cellBaseStyle = {
                        ...
                        cursor:
                          occupied || !roomReady ? 'default' : hasHold ? 'pointer' : 'pointer',
                        background: hasHold ? '#fff7ed' : !roomReady ? '#f8fafc' : undefined,
                      ...
                      return (
                        <Box
                          ...
                          onClick={
                            occupied || !roomReady
                              ? undefined
                              : () => handleEmptyCellClick(room.id, room.roomTypeId, d)
                          }
                          onKeyDown={
                            occupied || !roomReady
                              ? undefined
                          ...
                          role={occupied || !roomReady ? undefined : 'button'}
                          tabIndex={occupied || !roomReady ? -1 : 0}
                          aria-label={
                            occupied
                              ? undefined
                              : !roomReady
                                ? `${room.roomNumber} is ${room.operationalStatus || 'not ready'}`
                                : `Start booking for ${room.roomNumber} on ${isoDay(d)}`
                          }
                          data-testid={
                            occupied ? undefined : `calendar-empty-${room.id}-${isoDay(d)}`
                          }
                          style={cellBaseStyle}
                          onMouseEnter={
                            occupied || !roomReady
                    ...
                    {roomSpanFor(room.id).map((res) => {
                      ...
                      const startIdx = Math.max(
                        0,
                        Math.round((arr.getTime() - startDate.getTime()) / DAY_MS),
                      );
                      const endIdx = Math.min(
                        rangeDays,
                        Math.round((dep.getTime() - startDate.getTime()) / DAY_MS),
                      );
        ...
        <Group gap={16}>
          <Group gap={6}>
            <Box style={{ width: 12, height: 12, background: '#7d4dd6', borderRadius: 3 }} />
            <Text size="xs" c="#64748b">
              Confirmed
            </Text>
          </Group>
          <Group gap={6}>
            <Box style={{ width: 12, height: 12, background: '#059669', borderRadius: 3 }} />
            <Text size="xs" c="#64748b">
              Checked in
            </Text>
          </Group>
          <Group gap={6}>
            <Box style={{ width: 12, height: 12, background: '#f59e0b', borderRadius: 3 }} />
            <Text size="xs" c="#64748b">
              Pending
            </Text>
          </Group>
          <Group gap={6}>
            <Box style={{ width: 12, height: 12, background: '#94a3b8', borderRadius: 3 }} />
            <Text size="xs" c="#64748b">
              Checked out / cancelled
            </Text>
          </Group>
          <Group gap={6}>
            <Plus size={12} color="#7d4dd6" />
            <Text size="xs" c="#64748b">
              Empty cell: start a booking
            </Text>
          </Group>
        </Group>
      </Stack>
